...
# Function to classify text for multi-class scenario
def classify_text_prob_multi_class(text):
...
# Function to report URL to IWF
# Function to display a hyperlink to IWF for reporting
#def report_url_to_iwf(url):
   #print(f"URL reported to IWF: {url}")
# Display a hyperlink to IWF for reporting
    
    
    #iwf_report_url = "https://report.iwf.org.uk/org/"
   # st.markdown(f"Report this URL to IWF: [{iwf_report_url}]({iwf_report_url}{url})")
...
def link_classification_page():
    ...
    # Link Classification Page
    if st.button("Classify"):
        if user_input_link:
            prediction_probabilities = classify_text_prob_multi_class(user_input_link)
            csam_probability = prediction_probabilities[1]

            if csam_probability > 0.5:  # Set your threshold here
                st.error(f"URL Classified as CSAM with Probability: {csam_probability*100:.2f}%")
                st.link_button("Report to IWF", "https://report.iwf.org.uk/org/", help="Click to report to IWF")
            else:
                st.success(f"URL Classified as Benign with Probability: {prediction_probabilities[0]*100:.2f}%")
