...
def link_classification_page():
    st.header("Enter link or upload file having a list of links to classify")
    ...
    if st.button("Classify"):
        if user_input_link:
            ...
            # Read Excel file
            df = pd.read_excel(uploaded_file, header=None)  # Assuming no header

            # Perform classification for each link in the Excel file
            predictions = []
            csam_probabilities = []
            benign_probabilities = []
            for link in df.iloc[:, 0]:  # Assuming URLs are in the first column (index 0)
                prediction, probabilities = classify_text(link)
                predictions.append(prediction)
                csam_probabilities.append(probabilities[0][1] * 100)
                benign_probabilities.append(probabilities[0][0] * 100)
